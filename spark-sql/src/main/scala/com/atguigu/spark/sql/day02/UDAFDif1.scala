...
object UDAFDif1 {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql("select my_sum(salary) from p").show
    spark.close()
  }
}
class MySum extends UserDefinedAggregateFunction {
  ...
  // 缓冲区的数据类型  Double
  override def bufferSchema: StructType = StructType(StructField("salary",DoubleType)::Nil)
  // 最终聚合结果的类型   Double
  override def dataType: DataType = DoubleType
  // 确定性. 相同的输入是否返回相同输出  相同的输入有相同输出
